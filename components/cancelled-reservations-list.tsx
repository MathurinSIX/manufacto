...
interface CancelledReservationsListProps {
  ...
  registrationStatusMap: Record<string, RegistrationStatus>;
  error?: string | null;
}

const ITEMS_PER_PAGE = 5;

export function CancelledReservationsList({
  registrations,
  sessionsMap,
  registrationStatusMap,
  error,
}: CancelledReservationsListProps) {
    ...
    <>
      <div className="space-y-4">
        {paginatedRegistrations.map((reg) => {
          ...
          const cancelledDate = latestStatus?.created_at 
            ? new Date(latestStatus.created_at) 
            : null;
                ...
                <p className="text-xs text-destructive mt-1 font-medium">
                  ...
                  {cancelledDate && (
                    <span className="text-muted-foreground ml-1">
                      le {dateFormatter.format(cancelledDate)}
                    </span>
                  )}
                </p>
              </div>
