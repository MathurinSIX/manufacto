ALTER TABLE square_purchase
  ADD COLUMN IF NOT EXISTS activity_id UUID REFERENCES activity(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS session_id UUID REFERENCES session(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS reserved_start_ts TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS reserved_end_ts TIMESTAMPTZ;

CREATE INDEX IF NOT EXISTS idx_square_purchase_session_id
  ON square_purchase(session_id)
  WHERE session_id IS NOT NULL;

ALTER TABLE square_purchase
  DROP CONSTRAINT IF EXISTS square_purchase_reserved_interval_valid;

ALTER TABLE square_purchase
  ADD CONSTRAINT square_purchase_reserved_interval_valid
  CHECK (
    reserved_start_ts IS NULL
    OR reserved_end_ts IS NULL
    OR reserved_end_ts > reserved_start_ts
  );

-- Do not deduct credits when the user paid outside the credit wallet.
...
BEGIN
  ...
  IF v_payment_type IS DISTINCT FROM 'credits' THEN
    RETURN NEW;
  END IF;
